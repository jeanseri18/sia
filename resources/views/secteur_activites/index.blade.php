...
@include('sublayouts.until')
    <div class="container">
        ...
        <a href="{{ route('secteur_activites.create') }}" class="btn btn-primary mb-3">Ajouter un Secteur</a>
            ...
            <tbody>
                @foreach($secteurs as $secteur)
                    <tr>
                        ...
                        <td>{{ $secteur->nom }}</td>
                        <td>
                            <a href="{{ route('secteur_activites.edit', $secteur->id) }}" class="btn btn-warning">Modifier</a>
                            <form action="{{ route('secteur_activites.destroy', $secteur->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
