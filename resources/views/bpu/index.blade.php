...
<div class="col">
                    ...
                    <button class="btn btn-primary btn-sm form-control" onclick="editCategorie('{{ $categorie->id }}', '{{ $categorie->nom }}')">Modifier</button></div>
                    <div class="col">       <form action="{{ route('categories.destroy', $categorie->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
            ...
            <tr>
                <td colspan="11">
                    <form action="{{ route('souscategoriesbpu.store') }}" method="POST">
                        ...
                        <div class="col-md-2">  <button type="submit" class="btn btn-success btn-sm mt-2">Ajouter Sous-Catégorie</button>
                        </div> </div>
                    </form>
                </td>
            </tr>
            ...
            @foreach ($categorie->sousCategories as $sousCategorie)
                <tr bgcolor="#1F384C" class="text-white" height="40px">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="col-md-2">   <form action="{{ route('souscategoriesbpu.destroy', $sousCategorie->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                ...
                @foreach ($sousCategorie->bpus as $bpu)
                    ...
                    @foreach ($sousCategorie->bpus as $bpu)
                        <tr>
                            ...
                            <td>{{ $bpu->pu_ht }}</td>
                            <td>{{ $bpu->pu_ttc }}</td>
                            <td>    <a href="{{ route('bpus.edit', $bpu->id) }}" class="btn btn-warning btn-sm">Modifier</a>
...
<form action="{{ route('bpus.store') }}" method="POST">
    ...
    <input type="hidden" name="id_souscategorie" value="{{ $sousCategorie->id }}">
