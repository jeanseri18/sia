...
<div class="container">
    <h2>Liste des Corps de Métier</h2>
    <a href="{{ route('corpsmetiers.create') }}" class="btn btn-primary">Ajouter un Corps de Métier</a>
        ...
        <tbody>
            @foreach($corpsMetiers as $corpsMetier)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('corpsmetiers.edit', $corpsMetier->id) }}" class="btn btn-warning btn-sm">Modifier</a>
                        <form action="{{ route('corpsmetiers.destroy', $corpsMetier->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
